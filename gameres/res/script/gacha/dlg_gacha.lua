...
function p.ShowShopData( shopdata )
    ...
    --保存商城物品信息
    if #shopdata == 0  or shopdata == nil then
        WriteCon( "no item data" );
        return;
    end
    p.shopData = shopdata;
...
function p.ShowGiftPackData( giftdata )
    ...
    --保存商城礼包信息
    if #giftdata == 0  or giftdata == nil then
        WriteCon( "no gift data" );
        return;
    end
    p.giftData = giftdata;
...
function p.SetItemInfo( view , item , position, index)
    ...
    limitLab:SetText( row_limitLv .. GetStr( "buy_limit_level" ));
    ...
    if tonumber( lv ) >= tonumber( row_limitLv ) then
        ...
        if tonumber( row_limitNum ) ~= 0 then
            ...
            if tonumber( num ) ~= tonumber( row_limitNum ) then
                limitLab:SetText( GetStr( "limit_buy" )..row_limitNum.. GetStr( "count" ) .."("..num.."/"..row_limitNum..")");
                buyBtn:SetEnabled( true );
            else
                limitLab:SetText( GetStr( "is_bought" )..num.."/"..row_limitNum..GetStr( "count" ));
            end
        --没有购买限制
        else
    ...
    if tonumber( row_rebatePrice ) ~= 0 then
        priceLab:SetText( GetStr( "original_price" ).. row_price );
        priceLab:SetCenterline( true );
        priceLab:SetCenterlineColor( priceLab:GetFontColor() );
        
        rebatePriceLab:SetText( GetStr( "current_price" ) .. row_rebatePrice );
    --无折扣
    else 
        --原价处显示现价，修改颜色，现价处清零
        priceLab:SetText( GetStr( "current_price" ) .. row_price );
