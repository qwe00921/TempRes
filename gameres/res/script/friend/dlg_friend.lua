...
function p.ShowFriendList(friendlist)
    ...
    for i=1,#friendlist do
        ...
        lastDateLab:SetText( tostring(friend.Date) .. ToUtf8("天前"));
        if tonumber(friend.Date) == 0 then
            lastDateLab:SetText( ToUtf8("今天"));
        end
...
function p.ShowFriendApplyList(applylist)
	...
	for i=1,#applylist do
        ...
        applyDateLab:SetText( tostring(friendapply.Date) .. ToUtf8("天前"));
        if tonumber(friendapply.Date) == 0 then
            applyDateLab:SetText( ToUtf8("今天"));
        end
...
function p.ShowFriendRecommendList( recommendlist)
    ...
    for i=1,#recommendlist do
        ...
        lastDateLab:SetText( tostring(friendrecommend.Date) .. ToUtf8("天前"));
...
function p.ShowFriendUI(frienddata)
    ...
    --今日PK数
    p.pkNumLab:SetText(ToUtf8("今日切磋次数：")..tostring(frienddata.pk_num) .. "/" .. tostring(frienddata.pk_limit));
    --好友数
