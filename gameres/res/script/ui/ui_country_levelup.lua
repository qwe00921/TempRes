--------------------------------------------------------------
-- FileName:  country_levelup.lua
-- author: 此文件為自動生成，沒有作者。有問題請 @郭浩
--------------------------------------------------------------
ui_country_levelup = {};
local p =  ui_country_levelup;
p.ID_CTRL_BUTTON_RIGHT				    = 51;
p.ID_CTRL_BUTTON_LEFT					    = 50;
p.ID_CTRL_PICTURE_MIDDLE				  = 49;
p.ID_CTRL_LIST_BUILD					     = 56;
p.ID_CTRL_TEXT_UP2					       = 35;
p.ID_CTRL_TEXT_UP1					       = 34;
p.ID_CTRL_BUTTON_UP					      = 33;
p.ID_CTRL_PICTURE_UP_BG				   = 31;
p.ID_CTRL_PICTURE_EXP					    = 52;
p.ID_CTRL_PICTURE_EXPBG				   = 53;
p.ID_CTRL_TEXT_HEAD					      = 6;
p.ID_CTRL_PICTURE_HEAD				    = 42;
p.ID_CTRL_TEXT_RETURN					    = 44;
p.ID_CTRL_BUTTON_RETURN				   = 22;
p.ID_CTRL_PICTURE_MARK				    = 54;
p.ID_CTRL_PICTURE_BG					     = 55;
