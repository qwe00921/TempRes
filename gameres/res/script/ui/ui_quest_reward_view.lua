--------------------------------------------------------------
-- FileName:  quest_reward_view.lua
-- author: 此文件為自動生成，沒有作者。有問題請 @郭浩
--------------------------------------------------------------
ui_quest_reward_view = {};
local p =  ui_quest_reward_view;
p.ID_CTRL_VERTICAL_LIST_ITEMANDCHA	= 40;
p.ID_CTRL_TEXT_ITEM					      = 39;
p.ID_CTRL_PICTURE_36					     = 38;
p.ID_CTRL_TEXT_NEED_EXP_V				 = 36;
p.ID_CTRL_TEXT_EXP_UP					    = 35;
p.ID_CTRL_PICTURE_32					     = 34;
p.ID_CTRL_EXP_EXP						       = 33;
p.ID_CTRL_TEXT_EXP_V					     = 17;
p.ID_CTRL_TEXT_EXP					       = 6;
p.ID_CTRL_TEXT_SOUL_V					    = 32;
p.ID_CTRL_TEXT_SOUL					      = 31;
p.ID_CTRL_PICTURE_GHO					    = 30;
p.ID_CTRL_TEXT_MONEY_V				    = 18;
p.ID_CTRL_TEXT_MONEY					     = 5;
p.ID_CTRL_PICTURE_MONEY				   = 29;
p.ID_CTRL_TEXT_MISSION_NAME			= 2;
p.ID_CTRL_TEXT_CHAPTER				    = 3;
p.ID_CTRL_TEXT_STAR					      = 4;
p.ID_CTRL_PICTURE_REWARD_TEXT_BG		= 23;
p.ID_CTRL_PICTURE_BG					     = 28;
p.ID_CTRL_BUTTON_BG					      = 21;
