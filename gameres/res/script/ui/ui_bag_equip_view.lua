--------------------------------------------------------------
-- FileName:  bag_equip_view.lua
-- author: 此文件為自動生成，沒有作者。有問題請 @郭浩
--------------------------------------------------------------
ui_bag_equip_view = {};
local p =  ui_bag_equip_view;
p.ID_CTRL_BUTTON_SELL					    = 68;
p.ID_CTRL_BUTTON_STRENGTHEN			= 67;
p.ID_CTRL_PICTURE_MODE				    = 66;
p.ID_CTRL_TEXT_INFO					      = 84;
p.ID_CTRL_TEXT_83						       = 83;
p.ID_CTRL_PICTURE_INTRODUCE			= 64;
p.ID_CTRL_TEXT_SUB_PRO3				   = 88;
p.ID_CTRL_TEXT_SUB_PRO2				   = 87;
p.ID_CTRL_TEXT_SUB_PRO1				   = 86;
p.ID_CTRL_TEXT_SUB_PRO				    = 76;
p.ID_CTRL_TEXT_MAIN_PRO1				  = 85;
p.ID_CTRL_TEXT_MAIN_PRO				   = 75;
p.ID_CTRL_PICTURE_PROPERTY			 = 63;
p.ID_CTRL_PICTURE_TYPE				    = 71;
p.ID_CTRL_TEXT_LEVEL					     = 69;
p.ID_CTRL_PICTURE_STAR				    = 72;
p.ID_CTRL_PICTURE_EQUIP				   = 65;
p.ID_CTRL_BUTTON_CLOSE				    = 62;
p.ID_CTRL_COLOR_LABEL_EQUIP_NAME		= 82;
p.ID_CTRL_PICTURE_BG					     = 1;
